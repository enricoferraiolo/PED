...
\begin{itemize}
    ...
    \item \textbf{Special Tokens}: <PAD> (padding), <UNK> (unknown)
...
LSTM & \textbf{0.966} & 0.932 & 0.932 & 0.932 & 0.992 \\
CNN & \textbf{0.969} & \textbf{0.945} & 0.931 & 0.938 & \textbf{0.995} \\
...
\midrule
Logistic Regression & 0.750 & - & 0.535 & - \\
Random Forest & 0.829 & - & 0.731 & - \\
XGBoost & 0.800 & - & 0.693 & - \\
LSTM & 0.966 & - & 0.932 & - \\
CNN & 0.969 & - & 0.945 & - \\
TabTransformer & 0.962 & - & 0.959 & - \\
\bottomrule
\end{tabular}
\end{table}

\textit{Note: Actual poisoned data results should be inserted here after running training on the poisoned dataset.}

\subsection{Discussion on Adversarial Robustness}

\subsubsection{Expected Vulnerabilities}

Different model architectures exhibit varying degrees of robustness to data poisoning:

\textbf{Machine Learning Models}:
\begin{itemize}
    \item \textbf{Logistic Regression}: Most vulnerable due to its simple linear decision boundary. Poisoned examples directly shift the decision hyperplane.
    \item \textbf{Random Forest}: Moderate robustness through ensemble voting. Individual trees may overfit to poisoned samples, but majority voting provides some resilience.
    \item \textbf{XGBoost}: Similar to Random Forest but potentially more vulnerable due to boosting's focus on difficult examples, which may include poisoned samples.
\end{itemize}

\textbf{Deep Learning Models}:
\begin{itemize}
    \item \textbf{LSTM/CNN}: Should demonstrate greater robustness because they learn distributed representations over entire sequences rather than relying on individual keyword matches. However, if poisoning affects common patterns, they may still be vulnerable.
    \item \textbf{TabTransformer}: Potentially most vulnerable among DL models because it explicitly uses engineered features including keyword counts, which are directly targeted by the poisoning attack.
\end{itemize}

...
The framework implements six distinct classification models across three categories:
\begin{itemize}
    \item \textbf{Traditional ML}: Logistic Regression, Random Forest, and XGBoost
    \item \textbf{Sequential DL}: LSTM (Long Short-Term Memory) network
    \item \textbf{Convolutional DL}: CNN (Convolutional Neural Network)
    \item \textbf{Hybrid Transformer}: TabTransformer combining textual and tabular features
\end{itemize}
...
\textbf{Source}: Kaggle - "Phishing Emails" dataset (subhajournal/phishingemails)
...
\begin{itemize}
    ...
    \end{itemize}
    \item \textbf{Content}: Diverse phishing tactics including financial scams, fake account verification requests, lottery frauds, and credential harvesting attempts
...
\textbf{Source}: Kaggle - "Enron Email Dataset" (wcukierski/enron-email-dataset)
...
\subsubsection{Quality and Urgency Features}
...
The project implements a comprehensive framework encompassing three categories of models, each with distinct architectural characteristics and learning paradigms.
...
\begin{itemize}
    \item ML models: StandardScaler (zero mean, unit variance)
    \item DL models: StandardScaler for tabular features in TabTransformer
    \item Text embeddings: Learned during training
\end{itemize}
...
All models were successfully trained on the clean dataset using the specified training pipeline. Following are the results obtained on the held-out test set.
...
\begin{figure}[htbp]
    ...
    % [b] aligns subfigures by their bottom baseline so captions line up
    % {0.3\textwidth} sets width to 30% of the page text width
    \begin{subfigure}[b]{0.3\textwidth}
        \centering
        % Use width=\textwidth so the image fills its container (the 0.3 set above)
    ...
    \end{subfigure}
    \hfill % Adds spring-like space between subfigures
    % Subfigure 2 (Top Middle)
    \begin{subfigure}[b]{0.3\textwidth}
        \centering
    ...
    % leave a blank line to force a new row break
    \vspace{1em} % Optional: adds vertical space between rows

    % --- ROW 2 ---
    % Subfigure 4 (Bottom Left)
    \begin{subfigure}[b]{0.3\textwidth}
        \centering
    ...
    \caption{Training and Validation Curves for All Deep Learning Models on Clean Data. Each subfigure (\ref{fig:training_res_lr}-\ref{fig:training_res_tabtransformer}) displays confunsion matrix (on the left side) and the ROC curve on the right panel for the respective model.}
...
\begin{itemize}
    \item \textbf{Accuracy}: Overall correctness, important but insufficient alone
    \item \textbf{Precision}: $\frac{TP}{TP + FP}$ - Critical for minimizing false positives (legitimate emails marked as phishing)
    \item \textbf{Recall (Sensitivity)}: $\frac{TP}{TP + FN}$ - Critical for catching actual phishing emails
...
\begin{itemize}
    ...
    \item May be suffering from insufficient hyperparameter tuning
\end{itemize}
...
\begin{itemize}
    ...
    \item Perfect balance: 93.2\% precision and recall
...
\begin{enumerate}
    \item \textbf{DL Superiority}: Deep learning models achieve 13-22\% higher accuracy than ML models, demonstrating the importance of learning representations from raw text.
    ...
    \item \textbf{Precision-Recall Tradeoff}: The TabTransformer prioritizes precision (fewer false positives) at a slight cost to recall, which may be desirable in some deployment scenarios where user experience is critical.
...
The attack is sophisticated because:
\begin{itemize}
...
Theoretically, this poisoning strategy should:
\begin{enumerate}
    \item \textbf{Increase False Positives}: Models learn to associate common business terms with phishing
    \item \textbf{Reduce Precision}: More legitimate emails will be incorrectly classified as phishing
    \item \textbf{Maintain Recall}: Detection of actual phishing emails should remain relatively stable
    \item \textbf{Degrade User Trust}: Increased false positives may cause users to ignore warnings
\end{enumerate}

\subsection{Training on Poisoned Data}

The poisoned dataset (emails\_combined\_poisoned.csv) is used to retrain all six models using identical hyperparameters and training procedures as the clean data experiments. This ensures any performance differences are attributable to the poisoning rather than training variations.

\subsection{Results on Poisoned Data}

\begin{table}[H]
\centering
\caption{Model Performance Comparison: Clean vs. Poisoned Data}
\begin{tabular}{lcccc}
\toprule
...
As we anticipated, all models exhibit performance degradation when trained on poisoned data:
\begin{itemize}
    \item \textbf{Accuracy and Precision}: Accuracy and precision drop by a 10\-15\% margin across all models, with ML models suffering more pronounced declines.
    \item \textbf{Recall Stability}: Recall is the most affected metric, with DL models experiencing a more pronounced drop. This suggests that the poisoning strategy effectively confuses the models about legitimate emails containing targeted keywords.
    \item \textbf{F1-Score Reduction}: F1-scores decrease across the board, reflecting the combined impact on precision and recall.
    \item \textbf{ROC-AUC Reduction}: All models show decreased ROC-AUC scores, indicating a general decline in discriminative ability.
\end{itemize}
...
\begin{figure}[H]
    ...
    \caption{Training and Validation Curves for All Deep Learning Models on Poisoned Data. Each subfigure (\ref{fig:training_res_lr_poisoned}-\ref{fig:training_res_tabtransformer_poisoned}) displays confunsion matrix (on the left side) and the ROC curve on the right panel for the respective model.}
...
\begin{enumerate}
    ...
    \item \textbf{Robust Training Techniques}:
    \begin{itemize}
        \item Adversarial training with known poisoning strategies
        \item Certified robustness through randomized smoothing
        \item Meta-learning approaches that adapt to distribution shifts
    \end{itemize}
    
    \item \textbf{Model Ensembles}:
    \begin{itemize}
        \item Combine models trained on different data subsets
        \item Disagreement-based detection of suspicious samples
        \item Diverse architectures to avoid common failure modes
    \end{itemize}
    
    \item \textbf{Continuous Monitoring}:
    \begin{itemize}
        \item Track false positive rates over time
        \item User feedback loops to identify systematic errors
        \item Periodic model retraining with curated data
    \end{itemize}
\end{enumerate}

\subsubsection{Practical Implications}

The adversarial evaluation highlights critical considerations for deploying ML-based phishing detectors:
...
\begin{itemize}
    ...
    \item \textbf{Model Selection}: While DL models perform best on clean data, their robustness under adversarial conditions is an equally important selection criterion.
    ...
    \item \textbf{Adaptive Adversaries}: Real-world attackers will continuously probe for weaknesses. Systems must be designed for ongoing evolution rather than static deployment.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Comprehensive Comparison}: By evaluating six distinct models, the project identified that Deep Learning approaches substantially outperform traditional ML methods (15+ percentage points in accuracy) for this task.
...
\begin{enumerate}
    \item \textbf{Dataset Representativeness}: The Enron corpus, while valuable, represents older email patterns (2000-2002). Modern business communications may differ significantly.
    ...
    \item \textbf{Static Evaluation}: Models were trained and tested on fixed datasets. Real-world deployment requires adaptation to evolving phishing tactics through continual learning.
    
    \item \textbf{Interpretability}: Deep learning models, while accurate, are black boxes. Developing interpretable explanations for predictions would increase user trust and facilitate debugging.
    ...
    \item \textbf{Certified Robustness}: Future work could apply formal verification techniques to provide mathematical guarantees about model behavior under bounded adversarial perturbations.
\end{enumerate}
